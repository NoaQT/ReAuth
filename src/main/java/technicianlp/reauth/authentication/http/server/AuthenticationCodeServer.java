...
public final class AuthenticationCodeServer {
    ...
    public AuthenticationCodeServer(int port, String loginUrl, CompletableFuture<String> codeFuture, Executor executor) throws IOException, NoClassDefFoundError {
        ...
        LiteModReAuth.getLog().info("Starting local endpoint");
        server.start();
        LiteModReAuth.getLog().info("Started local endpoint");
    ...
    public final synchronized void stop(boolean immediate) {
        if (this.running) {
            this.running = false;
            if (!immediate) {
                LiteModReAuth.getLog().info("About to stop local endpoint");
                try {
                    TimeUnit.SECONDS.sleep(1);
                } catch (InterruptedException exception) {
                    LiteModReAuth.getLog().warn("Interrupted while waiting to stop local endpoint", exception);
                }
            }
            LiteModReAuth.getLog().info("Stopping local endpoint");
            this.stopServer.accept(immediate);
            LiteModReAuth.getLog().info("Stopped local endpoint");
        }
    }
}

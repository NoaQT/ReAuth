...
final class JceWorkaround {
    ...
    static void ensureUnlimitedCryptography() {
        try {
            if (Cipher.getMaxAllowedKeyLength("AES") != Integer.MAX_VALUE) {
                LiteModReAuth.getLog().warn("Cryptography is restricted in this Java installation");
                if (!MojangJavaFix.java8) {
                    LiteModReAuth.getLog().warn("Cryptography is likely deliberately restricted!");
                }
                removeCryptographyRestrictions();
                if (Cipher.getMaxAllowedKeyLength("AES") != Integer.MAX_VALUE) {
                    LiteModReAuth.getLog().error("Failed to remove cryptography restriction");
                } else {
                    LiteModReAuth.getLog().info("Cryptography restriction removed successfully");
                }
            }
    ...
    private static void removeCryptographyRestrictions() {
        try {
            ...
            LiteModReAuth.getLog().error("Exception removing cryptography restrictions", e);
        }
    }
}
